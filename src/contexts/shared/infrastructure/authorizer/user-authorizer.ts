...
export class JWTUserAuthorizer implements IAuthorizer<Request, Response, NextFunction> {
    public authorize : Middleware = async (req:any,res:Response, next: NextFunction):Promise<void> => {
        ...
        try {
            ...
            if(validRoles.includes(payload.role as any) && (payload.scope.includes(TokenScope.ADMIN_ACCESS) || payload.scope.includes(TokenScope.BUYER_ACCESS) || payload.scope.includes(TokenScope.BUYER_ACCESS))) {
                req.user = payload;
                return next()
            } else {
